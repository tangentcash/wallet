...
export function AssetName(props: { asset?: AssetId, size?: string, badgeSize?: number }) {
  ...
  return (
    <Tooltip content={ Readability.toAssetName(props.asset, true) + ' blockchain' }>
      <Flex align="center" gap="1">
        <Text as="div" size={size} weight="light">{fake ? Readability.toAssetSymbol(props.asset) : Readability.toAssetName(props.asset)}</Text>
        { contractAddress && <Icon path={mdiCheckDecagram} color="var(--sky-9)" size={props.badgeSize || 0.7} style={{ transform: 'translateY(-2px)' }}></Icon> }
      </Flex>
    </Tooltip>
  )
}
